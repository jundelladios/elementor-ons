...
if( !function_exists( 'autogenTemplateAppedScript' ) ) {
    function autogenTemplateAppedScript() {
		global $remoteURL, $request, $apidata, $directoryURI, $parent, $themeVersion, $galleryLayout, $galleryColumn, $galleryFancybox, $wp;
		ob_start();
		?>
...
if( isset( $apidata->jsondata->headerinject ) && $apidata->jsondata->headerinject ):
add_action('wp_head', 'landing_page_generator_wp_header');
function landing_page_generator_wp_header() {
	global $apidata;
	echo $apidata->jsondata->headerinject;
};
endif;


if( isset( $apidata->jsondata->footerinject ) && $apidata->jsondata->footerinject ):
add_action('wp_footer', 'landing_page_generator_wp_footer', 100);
function landing_page_generator_wp_footer() {
	global $apidata;
	echo $apidata->jsondata->footerinject;
};
endif;